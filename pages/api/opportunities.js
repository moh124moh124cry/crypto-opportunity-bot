...
function calculateOpportunity(ticker) {
  ...
  const opportunityScore = round(
    clamp(momentumScore + liquidityScore + positionScore, 0, 100)
  );
...
function calculateFinalScore(opportunityScore, technicalScore) {
  ...
  const finalOpportunityScore = round(
    clamp((opportunityScore * 0.5) + (technicalScore * 0.5), 0, 100)
  );
  ...
  try {
    ...
    if (!response.ok || !Array.isArray(data)) {
      return {
        ...
        technicalScore: null,
        technicalLevel: "unavailable",
        technicalError: data?.msg || "Unable to fetch candle data",
      };
    }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (symbols.length === 1) {
      tickerParams.set("symbol", symbols[0]);
    } else {
      tickerParams.set("symbols", JSON.stringify(symbols));
    }
    ...
    if (!tickerResponse.ok) {
      return res.status(tickerResponse.status).json({
        ok: false,
        source: "Binance",
        error: tickerData?.msg || "Binance ticker request failed",
        code: tickerData?.code ?? null,
      });
    }
    ...
    const technicalResults = await Promise.all(
      tickers.map((ticker) => fetchTechnical(ticker.symbol, controller.signal))
    );

    const technicalBySymbol = new Map(
      technicalResults.map((item) => [item.symbol, item])
    );

    const opportunities = tickers
      .map((ticker) => {
        const technical = technicalBySymbol.get(ticker.symbol) || {
          technicalScore: null,
          technicalLevel: "unavailable",
        };
        ...
        return {
          ...
          opportunityLevel: ticker.opportunityLevel,
          trendBias: ticker.trendBias,
          rsi14: technical.rsi14 ?? null,
          rsiState: technical.rsiState ?? "unavailable",
          ema20: technical.ema20 ?? null,
          ema50: technical.ema50 ?? null,
          emaTrend: technical.emaTrend ?? "neutral",
          technicalScore: technical.technicalScore,
          technicalLevel: technical.technicalLevel,
          technicalError: technical.technicalError ?? null,
          ...finalScore,
        };
      })
      .sort((a, b) => {
        const scoreA = a.finalOpportunityScore ?? -1;
        const scoreB = b.finalOpportunityScore ?? -1;
    ...
    return res.status(200).json({
      ...
      finalScoreModel: "opportunity-v1-50pct-technical-v1-50pct",
      weights: {
        opportunityScore: 0.5,
        technicalScore: 0.5,
      },
      disclaimer:
        "Final Opportunity Score is a market-filtering heuristic for signal research and paper trading. It is not a trading recommendation or profit guarantee.",
      opportunities,
